...
export default async function JournalsPage({ searchParams }: { searchParams: { view?: string } }) {
  ...
  if (!view || view === "") {
    ...
    console.log("🔍 Debug - Latest issue articles:", JSON.stringify(latestIssueArticles, null, 2))
                ...
                <div className="md:col-span-2 space-y-6">
                  ...
                  {latestIssueArticles.length > 0 ? (
                    latestIssueArticles.map((article, index) => {
                      // Debug: Log each article's author data
                      console.log(`🔍 Debug - Article ${index + 1} author data:`, {
                        title: article.title,
                        Author: article.Author,
                        authors: article.authors,
                        hasAuthor: !!article.Author,
                        hasAuthors: !!(article.authors && article.authors.length > 0)
                      })
                              ...
                              <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                                <div className="flex items-center gap-1">
                                  ...
                                  <div className="flex flex-wrap">
                                    {/* Debug display showing what we have */}
                                    <div className="text-xs bg-yellow-100 px-2 py-1 rounded mr-2">
                                      Debug: Author={article.Author?.name || 'null'}, 
                                ...
                                <div className="mt-4">
                                  <details className="group">
                                    <summary className="cursor-pointer text-sm font-medium text-primary hover:underline">
                                      Preview content with images ({article.images.length} images)
                                    </summary>
